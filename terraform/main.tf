...
variable "budget_limit_usd" { type = number default = 5000 }
variable "email" { type = string description = "Budget notification email" default = "finops@example.com" }
...
resource "aws_ce_anomaly_subscription" "finops" {
  ...
  subscriber { type = "EMAIL" address = var.email }
